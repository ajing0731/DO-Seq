...
# get the FC of all NAD-RNAs during drosophila development
res.ls <- getEnrichment(Enone, slot = 'sample', filter = FALSE)
nad_df1_all <- reduceRes(res.ls, fc.col = 'logFC')
nad_df1_all$Group <- gsub('\\..*', '', nad_df1_all$Group)
nad_df1_all$Group <- factor(nad_df1_all$Group, levels = unique(nad_df1_all$Group))
nad_df1_all <- subset(nad_df1_all, nad_df1_all$GeneID %in% unique(nad_df1$GeneID))

write.csv(nad_df1,"nad_df1.csv")
write.csv(nad_df1_all,"nad_df1_all.csv")